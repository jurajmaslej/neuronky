...
class MLP():
    ...
    def __init__(self, dim_in, dim_hid, dim_out):
        ...
        self.W_hid = np.random.rand(dim_hid, dim_in)
        self.W_out = np.random.rand(dim_out, dim_hid)
    ...
    def forward(self, x):
        a = np.dot(self.W_hid, x)
        h = self.f_hid(a)
        b = np.dot(self.W_out, h)
        y = self.f_out(b)
    ...
    def backward(self, x, d):
        ...
        g_hid = np.dot(self.W_out.T, g_out) * self.df_hid(a)
